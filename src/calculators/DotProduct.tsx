...
const DotProduct: React.FC = () => {
  ...
  const performOperation = () => {
    ...
    try {
      ...
      if (vectorA.length !== vectorB.length && operation !== 'norm') {
      ...
      stepList.push({
        step: '📊 Input Vectors:',
        explanation: 'The input vectors for the vector operation.'
      });
      stepList.push({
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-blue-50 p-4 rounded-lg border border-indigo-100">
        ...
        <p className="text-sm text-indigo-600 mt-1">
          Calculate dot product, cross product, angle between vectors, projection, and more with step-by-step explanations
        </p>
      </div>
      ...
      {vectorA.length !== vectorB.length && operation !== 'norm' && (
        <div className="p-2 bg-yellow-50 rounded-lg border border-yellow-200 text-xs text-yellow-700">
          ...
          {operation === 'cross' && ' Cross product requires 3D vectors.'}
        </div>
      )}
      ...
      <div className="text-sm text-slate-500 bg-slate-50 p-3 rounded-lg border border-slate-100">
        ...
        <p className="mt-2 text-xs text-slate-500">
          ...
          Dot product, angle, and projection require vectors of the same dimension.
        </p>
      </div>
    </div>
  );
